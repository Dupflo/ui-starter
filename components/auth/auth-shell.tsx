...
export function AuthShell({
  ...
  return (
    <div className="relative flex min-h-screen items-center justify-center bg-pine p-6 sm:p-10">
      ...
      <div className="absolute right-4 top-4 sm:right-6 sm:top-6 print-hide">
        <LocaleMenu />
      </div>
      <div className="w-full max-w-sm">
        <div className="mb-6 flex justify-center print-hide">
          <Logo />
        </div>
        <div className="rounded-2xl bg-input p-8 shadow-float">{children}</div>
        <p className="mt-5 text-center font-mono text-2xs uppercase tracking-caps text-on-pine print-hide">
          {footer}
        </p>
      </div>
    </div>
