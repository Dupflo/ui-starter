...
| `Avatar`                        | `src?` · `name` (obligatoire) · `size` = `sm` \| `md` \| `lg` · `decorative?`                                                                                                 | Avatar personne : image si `src`, sinon initiales. Voir § Avatar                                                                                  |
...
- **Écrit à la main** (s18-ui-kit-polish), périmètre strict : image quand `src` est fourni, initiales sinon — rien d'autre (pas de badge de présence, pas de groupe empilé ; une story ultérieure si le besoin apparaît).
- **`name` est obligatoire**, toujours : c'est à la fois la source des initiales et, sauf `decorative`, le nom accessible — un avatar sans nom sur lequel retomber est exactement la classe de défaut que ce repo évite ailleurs (nom accessible dépendant d'un prop optionnel absent, cf. le combobox de s14 avant son correctif).
- **Deux états d'accessibilité, au choix de l'appelant** : par défaut (`decorative` absent/`false`), l'avatar **porte** le nom accessible (`alt={name}` sur `<img>`, ou `role="img"` + `aria-label={name}` sur le repli initiales) — pour un avatar affiché seul, sans nom visible à côté. `decorative` (booléen) retire l'avatar de l'arbre d'accessibilité (`alt=""` sur `<img>`, `aria-hidden="true"` sur le repli initiales) — pour un avatar affiché À CÔTÉ d'un nom déjà visible (une ligne de tableau, typiquement) : sans `decorative`, un lecteur d'écran annoncerait le même nom deux fois. Le bloc « Utilisateurs » (ci-dessous) illustre le deuxième cas ; la démo de la galerie, le premier.
- **`<img>` simple, pas `next/image`** : la valeur de `next/image` (redimensionnement responsive, allowlist de sources distantes, lazy loading) cible des images plus grandes et récupérées sur le réseau — rien de tout cela n'aide un avatar de 32 à 56 px dont la `src` de démo est un data-URI SVG local déjà minuscule, et qui en usage réel serait le plus souvent une URL déjà signée depuis le stockage objet (Supabase Storage), n'ayant besoin d'aucune optimisation supplémentaire.
- **Images de démonstration** (`components/gallery/avatar-fixtures.ts`) : des **SVG inline en data-URI**, jamais un fichier dans `public/` — tout fichier y part dans chaque fork, alors que la galerie n'existe qu'en dev et en démo (même arbitrage que les polices retirées en s10). Les couleurs sont des copies littérales des tokens `@theme` (pas de `var(--color-…)` possible : un SVG en data-URI dans un `<img>` est un document isolé, sans accès aux custom properties CSS de la page parente) — `check-design-tokens` les verrait et les bloquerait à raison ; elles sont donc explicitement autorisées via son mécanisme `design-tokens-allow` (même précédent que les couleurs de marque Google dans `google-button.tsx`), et croisées avec `app/globals.css` par `avatar-fixtures.test.ts` pour qu'un token qui change ne puisse pas y dériver silencieusement. Les couleurs réellement rendues ont aussi été vérifiées sur le rendu (voir le rapport de la story) — un lint vert n'est pas une preuve suffisante ici (même piège que les couleurs par défaut de Recharts, s14).
...
- **Bloc « Utilisateurs »** (`components/gallery/data-table-users-demo.tsx`) : une **composition** sur `DataTable`, pas un second composant — une colonne `Avatar` dédiée (`decorative`, voir § Avatar : le nom est déjà visible dans la colonne « Utilisateur » de la même ligne, deux rows sur quatre avec une vraie image, les deux autres démontrant le repli initiales), `Badge` pour le statut, `Button` pour l'action de ligne. Client Component dédié : `columns` porte des fermetures `cell` (des fonctions), qui ne peuvent pas traverser la frontière Server→Client en tant que prop React/Next — le même problème que le `registration` de `TextField`.
- **Démo du primitif `DataTable`** (`components/gallery/primitives-section.tsx`) : 6 colonnes (texte, texte non triable, texte, nombre, date, statut) — assez de TYPES pour que le tri se comporte visiblement différemment selon le type (s18-ui-kit-polish, annotation `mtlqxys25i7`). Le défilement horizontal, lui, était déjà déclenché avant cette story (3 colonnes texte suffisaient déjà à dépasser la largeur du wrapper en mobile) ; les colonnes ajoutées élargissent ce débordement, elles ne le rendent pas démonstrable pour la première fois — et à ≥768px, ni l'ancienne ni la nouvelle version ne défile. Pas de colonne avatar ici, délibérément : cette démo est le primitif nu, rendu directement depuis un Server Component sans wrapper client, et une cellule `Avatar` a besoin d'un `cell` (une fonction) qui ne peut pas traverser la frontière Server→Client sans devenir un nouvel escape hatch — la colonne avatar reste dans la composition « Utilisateurs » ci-dessus, qui paie déjà ce coût pour ses cellules `Badge`/`Button`.
...
- ✅ Composer `components/ui/*` (`Button`, `Title`, `Text`, `Badge`, `Avatar`, `Card`, `Modal`, `Select`, `Combobox`, `Container`, `SectionLabel`, `ChartLine`, `ChartBar`, `ChartDonut`, `DataTable`).
